...
$modelLabel = new \backend\models\GameMailSend();
?>
        ...
        <div class="box-body">
          <div id="example1_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
            <!-- row start search-->
          	<div class="row">
          	<div class="col-sm-12">
                <?php ActiveForm::begin(['id' => 'game-mail-send-search-form', 'method'=>'get', 'options' => ['class' => 'form-inline'], 'action'=>Url::toRoute('game-mail-send/index')]); ?>     
            ...
            foreach ($models as $model) {
                ...
                echo '      <a id="edit_btn" onclick="editAction(' . $model->id . ')" class="btn btn-primary btn-sm" href="#"> <i class="glyphicon glyphicon-edit icon-white"></i>审核</a>';
                if ($model->status==3){
                	echo '      <a id="send_btn" onclick="sendAction(' . $model->id . ')" class="btn btn-primary btn-sm" href="#"> <i class="glyphicon glyphicon-edit icon-white"></i>发送</a>';
                }
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
                <?php $form = ActiveForm::begin(["id" => "game-mail-send-form", "class"=>"form-horizontal", "action"=>Url::toRoute("game-mail-send/save")]); ?>                      
          ...
          <div id="title_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="title" name="GameMailSend[title]" placeholder="必填" />
              </div>
          ...
          <div id="content_div" class="form-group">
              ...
              <div class="col-sm-10">
              	<textarea class="form-control" id="content" name="GameMailSend[content]" placeholder="必填" rows="3"></textarea>
              </div>
              <div class="clearfix"></div>
          </div>
          ...
          <div id="type_value_div" class="form-group">
              ...
              <div class="col-sm-10">
              	<textarea class="form-control" id="type_value" name="GameMailSend[type_value]" placeholder="" rows="5"></textarea>
              </div>
              <div class="clearfix"></div>
          </div>
          ...
          <div id="attach_div" class="form-group">
              ...
              <div class="col-sm-10">
              	<textarea class="form-control" id="attach" name="GameMailSend[attach]" placeholder="" rows="5"></textarea>
              </div>
              <div class="clearfix"></div>
          </div>
          ...
          <div id="status_div" class="form-group">
              ...
              <div class="col-sm-10">
              	<select class="form-control" id="status" name="GameMailSend[status]">
          ...
          <div id="record_time_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="record_time" name="GameMailSend[record_time]" placeholder="必填" />
              </div>
              <div class="clearfix"></div>
          </div>
          ...
          <div id="record_user_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="record_user" name="GameMailSend[record_user]" placeholder="必填" />
              </div>
              <div class="clearfix"></div>
          </div>
          ...
          <div id="update_time_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="update_time" name="GameMailSend[update_time]" placeholder="" />
              </div>
              <div class="clearfix"></div>
          </div>
          ...
          <div id="update_user_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="update_user" name="GameMailSend[update_user]" placeholder="" />
              </div>
              <div class="clearfix"></div>
          </div>
			...
			<div class="modal-footer">
				<a href="#" class="btn btn-default" data-dismiss="modal">关闭</a> <a
					id="edit_dialog_ok" href="#" class="btn btn-primary">通过</a>
					<a id="edit_dialog_return" href="#" class="btn btn-danger">驳回</a>
			</div>
		</div>
	</div>
</div>
...
function orderby(field, op){
 ...
 function searchAction(){
		$('#game-mail-send-search-form').submit();
	}
 function viewAction(id){
		initModel(id, 'view', 'fun');
	}
 function sendAction(id){
     	var url="<?=Url::toRoute('game-mail-send/send')?>";
     	var res=http_post(url,"id="+id+'&_csrf='+_csrf);
     	if(res.errno==0){
			var msg_type="success";
			var interval=setInterval(function(){
		         	clearInterval(interval);
		         	admin_tool.alert('msg_info',res.msg,msg_type);
 ...
 function initEditSystemModule(data, type){
	...
	$('#edit_dialog_return').addClass('hidden');
	}
	else{
      ...
      $("#update_user").parent().parent().hide();
		...
		$('#edit_dialog_return').removeClass('hidden');
		}
...
function initModel(id, type, fun){
	...
	$.ajax({
		   type: "GET",
		   url: "<?=Url::toRoute('game-mail-send/view')?>",
		   data: {"id":id},
		   cache: false,
		   dataType:"json",
...
function deleteAction(id){
	...
	if(ids.length > 0){
		admin_tool.confirm('请确认是否删除', function(){
		    $.ajax({
				   type: "GET",
				   url: "<?=Url::toRoute('game-mail-send/delete')?>",
				   data: {"ids":ids},
				   cache: false,
				   dataType:"json",
				   error: function (xmlHttpRequest, textStatus, errorThrown) {
...
$('#edit_dialog_ok').click(function (e) {
    e.preventDefault();
    $('#game-mail-send-form #status').val(3);
	$('#game-mail-send-form').submit();
});

$('#edit_dialog_return').click(function (e) {
    e.preventDefault();
    $('#game-mail-send-form #status').val(2);
	$('#game-mail-send-form').submit();
});
...
$('#game-mail-send-form').bind('submit', function(e) {
	e.preventDefault();
	var id = $("#id").val();
	var action = id == "" ? "<?=Url::toRoute('game-mail-send/create')?>" : "<?=Url::toRoute('game-mail-send/update')?>";
    $(this).ajaxSubmit({
    	type: "post",
    	dataType:"json",
    	url: action,
